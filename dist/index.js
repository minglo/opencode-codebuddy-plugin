...
const CONFIG = {
    ...
    conversationIdMapMax: Number(process.env.CODEBUDDY_CONVERSATION_ID_MAP_MAX) || 1000,
    authMode: (process.env.CODEBUDDY_AUTH_MODE || "auto").toLowerCase(),
    apiKey: process.env.CODEBUDDY_API_KEY || "",
    internetEnv: (process.env.CODEBUDDY_INTERNET_ENVIRONMENT || "internal").toLowerCase(),
...
function effectiveAuth(stored) {
    const mode = pickAuthMode(stored);
    if (mode === "api") {
        if (CONFIG.apiKey)
            return { type: "api", key: CONFIG.apiKey };
        if (stored?.type === "api" && stored.key)
...
const sessionConversationIds = new LRUMap(CONFIG.conversationIdMapMax);
...
async function fetchRemoteModels(accessToken, signal) {
    const headers = {
...
function getOrCreateConversationId(sessionId) {
    if (!CONFIG.stableConversationId)
...
function resetConversationId(sessionId) {
    if (!sessionId)
        return;
    sessionConversationIds.delete(sessionId);
}
function buildRequestHeaders(sessionId, modelId) {
 ...
 */
function createSSEBufferedStream(body) {
    ...
    const hasFlushTrigger = (s) => s.includes("\n") || /[。！？.!?；;，,：:]$/.test(s.trimEnd());
        ...
        transform(chunk, controller) {
            ...
            for (const rawLine of lines) {
                ...
                try {
                    ...
                    if (reasoningBuf) {
                        controller.enqueue(encoder.encode(`data: ${JSON.stringify({ choices: [{ delta: { reasoning_content: reasoningBuf } }] })}\n`));
                        reasoningBuf = "";
                    }
                    if (contentBuf) {
                        controller.enqueue(encoder.encode(`data: ${JSON.stringify({ choices: [{ delta: { content: contentBuf } }] })}\n`));
                        contentBuf = "";
                    }
                    controller.enqueue(encoder.encode(line + "\n"));
                    continue;
                ...
                if ((hasReasoning || hasReasoningAlt) && !hasContent && !hasToolCalls && !finishReason) {
                    ...
                    reasoningBuf += chunk;
                    const shouldFlush = reasoningBuf.length >= threshold || hasFlushTrigger(reasoningBuf);
                    if (shouldFlush) {
                        const outPayload = {
                            ...payload,
                            choices: [
...
async function refreshAccessToken(refreshToken) {
    ...
    try {
        ...
        if (!response.ok) {
            const text = await response.text();
            console.error(`[codebuddy] token refresh failed: ${response.status} - ${text}`);
            return null;
        }
        const data = (await response.json());
        if (data.code !== 0) {
            console.error(`[codebuddy] token refresh bad code: ${data.code} - ${JSON.stringify(data)}`);
            return null;
        }
...
export const CodeBuddyAuthPlugin = async (input) => {
    return {
        async config(config) {
            ...
            if (mode === "api") {
                authAvailable = true;
                console.log("[codebuddy] api key mode — using models from opencode.json config");
                discovered = [];
            }
            ...
            if (!authAvailable) {
                if (mode === "api") {
                    console.log("[codebuddy] api key mode requested but no key found — set CODEBUDDY_API_KEY env or run `/connect codebuddy` (falling back to auto model)");
                }
                else {
                    console.log(`[codebuddy] no oauth token in ${authPath} — run \`/connect codebuddy\` to log in (falling back to auto model)`);
                }
            }
            else if (mode === "oauth" && discovered.length === 0) {
                console.log("[codebuddy] no models discovered from /v3/config (auth may be expired — try `/connect codebuddy` to re-authenticate)");
            }
            ...
            for (const m of discovered) {
                ...
                if (auto.options && !existing.options)
                    merged.options = auto.options;
                if (auto.variants && !existing.variants)
                    merged.variants = auto.variants;
                if (auto.limit && !existing.limit)
                    merged.limit = auto.limit;
                if (auto.attachment && existing.attachment === undefined)
                    merged.attachment = true;
                if (auto.tool_call && existing.tool_call === undefined)
                    merged.tool_call = true;
                models[m.id] = merged;
            }
        },
        async event({ event }) {
            ...
            async loader(getAuth, _provider) {
                return {
                    ...
                    async fetch(url, init) {
                        ...
                        if (activeAuth.type === "oauth" &&
                            ...
                            if (refreshed?.accessToken) {
                                ...
                                try {
                                    ...
                                    console.error("[codebuddy] failed to persist refreshed token, continuing in-memory:", err);
                                }
                        ...
                        if (CONFIG.sseBufferEnabled &&
                            response.body &&
                            response.headers.get("content-type")?.includes("text/event-stream")) {
                            const bufferedBody = createSSEBufferedStream(response.body);
